...
getNewChar = (ind, num)=> { // finds new character based on index and number given
    function newIndex(i,n) { // callback function for return
        ...
        do { // run through function recursively
...
const caesar = function(str, num) {
    ...
    Array.from(str).forEach(function(current){ // convert string to array and loop through it
        ...
        // determine either new character or glyph to push to target array
        !isNaN(parseInt(ind)) ? target.push(getNewChar(ind, num)) : target.push(ind);
    });
